...
$testFiles = Get-ChildItem -Path . -Include "*.test.js" -Recurse | Where-Object { $_.FullName -notmatch "node_modules" }
...
Set-Content "README.md" -Value $honestReadme -Encoding UTF8
...
foreach ($term in $mysticalTerms) {
    $files = Get-ChildItem -Path . -Include "*.js", "*.md", "*.json" -Recurse | 
