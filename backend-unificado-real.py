...
class BackendUnificadoReal:
    ...
    async def capture_srona_data(self):
        ...
        logger.info(" Capturando datos de SRONA API...")
    ...
    async def capture_frontend_data(self):
        ...
        logger.info(" Capturando datos de Frontend API...")
        ...
        # Capturar métricas cuánticas del frontend
    ...
    def show_system_stats(self, cycle_time: float):
        ...
        if self.start_time:
            ...
            logger.info("=" * 60)
            logger.info(f"[DATA] ESTADÍSTICAS DEL SISTEMA UNIFICADO REAL")
            ...
            logger.info(f" Servicios saludables: {self.metrics['services_healthy']}/{len(SERVICES)}")
            logger.info(f"[RELOAD] Ciclo: {cycle_time:.2f}s")
            logger.info(f"[DATA] Datos en caché: {len(self.data_cache['market_data'])} endpoints")
...
async def main():
    ...
    print("=" * 70)
    print("[DATA] Integración de servicios reales del sistema QBTC")
    print(" QBTC Core (4602) - SRONA API (4601) - Frontend (4603)")
    print("[UP] Vigo Futures (8002) - Sin simulaciones")
    print("[ENDPOINTS] Consolidación real de oportunidades de trading")
    print("=" * 70)
