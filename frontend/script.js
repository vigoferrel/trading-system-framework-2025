...
function initializeDOMElements() {
    ...
    if (!connectionStatusEl) {
        console.warn('[WARNING] Elemento connectionStatus no encontrado');
    }
    if (!lastUpdateEl) {
        console.warn('[WARNING] Elemento lastUpdate no encontrado');
    }
}
            ...
            try {
                ...
                if (body) { 
                    ...
                    const rowsToShow = prioLowSpr
                        ...
                        : rowsBase;
                    rowsToShow.forEach((r,i)=>{ const tr=document.createElement('tr'); if ((r.volPct||0)>=hv) tr.classList.add('rank-highvol'); if (prioLowSpr && (r.volPct||0)>=hv) tr.classList.add('rank-lowspr'); const crit = Number(document.getElementById('wsCritSpreadBps')?.value||'0'); const critHit = crit>0 && (r.spreadBps||0)>crit; tr.innerHTML=`<td>${i+1}</td><td>${r.sym}</td><td>${r.score.toFixed(3)}</td><td>${(r.volPct||0).toFixed(2)}%</td><td>${(r.vol||0).toLocaleString()}</td><td>${r.coh.toFixed(2)}</td><td>${r.edge.toFixed(2)}</td><td>${(r.spreadBps||0).toFixed(2)}</td><td>${critHit?'[WARNING]':''}</td>`; body.appendChild(tr); }); 
                }
            } catch(_) {}
        });
...
function updateConnectionStatus(status) {
    ...
    if (!connectionStatusEl) {
        console.warn('[WARNING] connectionStatusEl no disponible, intentando inicializar...');
        initializeDOMElements();
        if (!connectionStatusEl) {
            console.error('[ERROR] No se pudo encontrar connectionStatusEl');
            return;
        }
    }
    ...
    if (!icon || !text) {
        console.warn('[WARNING] Elementos icon o text no encontrados en connectionStatus, recreando estructura');
    ...
    switch (status) {
        ...
        console.warn('[WARNING] Error actualizando elementos de conexión:', error);
        // Fallback: recrear la estructura completa
        const statusText = status === 'connected' ? 'Conectado' : 
                          status === 'disconnected' ? 'Desconectado' : 
                          status === 'connecting' ? 'Conectando...' : 'Desconocido';
...
function buildZStrategyPreview(symA, symB){
    try {
        ...
        // ADF sobre el spread (d ~ spread): test de raíz unitaria simple con lags
        ...
        box.innerHTML = `=${beta.toFixed(3)}  z=${z.toFixed(2)}  Dirección: <strong>${dir}</strong>  ADF t=${adf.t.toFixed(2)} (${decision})`;
...
function adfTest(series, maxLag){
    try {
        ...
        // construir regresores: y_{t-1}, y_{t-1}..y_{t-k}
...
function showAbout() {
    ...
    modalBody.innerHTML = `
        <div class="about-content">
            ...
            <h4>[START] Características Principales:</h4>
            <ul>
            ...
            <h4>[DATA] Métricas del Sistema:</h4>
...
function showDocumentation() {
    ...
    modalBody.innerHTML = `
        <div class="documentation-content">
            <h3>[LIST] Documentación Disponible</h3>
            ...
            <h4>[API] Endpoints API Principales:</h4>
            <ul>
            ...
            <h4>[ALERT] Comandos de Emergencia:</h4>
            <ul>
...
function showSettings() {
    ...
    modalTitle.textContent = ' Configuración del Sistema';
    ...
    modalBody.innerHTML = `
        <div class="settings-content">
            ...
            <h4>[FAST] Configuración Rápida:</h4>
            <ol>
            ...
            <h4>[ENDPOINTS] Parámetros del Sistema:</h4>
            <ul>
...
console.log('[START] Quantum Trading Dashboard - Production Ready');