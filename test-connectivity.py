    ...
    try:
        async with session.get(endpoint["url"], timeout=10) as response:
            ...
            if response.status == 200:
                print(f"[OK] {endpoint['name']}: OK ({response_time:.2f}s)")
                return True
            else:
                print(f"[WARNING] {endpoint['name']}: Status {response.status}")
                return False
    except Exception as e:
        print(f"[ERROR] {endpoint['name']}: ERROR - {str(e)}")
        return False

async def test_all():
    print("[SEARCH] PRUEBA DE CONECTIVIDAD")
    print("=" * 50)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        total = len(results)
        print(f"\n[DATA] RESUMEN: {successful}/{total} exitosos ({(successful/total)*100:.1f}%)")
