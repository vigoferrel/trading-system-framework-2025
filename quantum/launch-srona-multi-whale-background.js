...
class Launcher {
    ...
    async launch() {
        try {
            console.log('[START] Lanzando SRONA MULTI WHALE en segundo plano...');
            console.log('================================================');
            ...
            if (this.isProcessRunning()) {
                console.warn('[WARNING] Ya hay un proceso en ejecución');
                return false;
            }
            ...
            console.log(`[OK] Proceso lanzado exitosamente`);
            console.log(`[DATA] PID: ${this.childProcess.pid}`);
            console.log(` Log file: ${this.config.logFile}`);
            console.log(`[UP] PID file: ${this.config.pidFile}`);
            console.log(`[TIME] El sistema comenzará a analizar datos en segundos...`);
            
            return true;
            
        } catch (error) {
            console.error('[ERROR] Error lanzando proceso:', error);
            return false;
        }
    }
    ...
    checkEnvironmentVariables() {
        ...
        console.log('[OK] Variables de entorno verificadas');
    }
    ...
    isProcessRunning() {
        try {
            if (fs.existsSync(this.config.pidFile)) {
                ...
                // Verificar si el proceso está activo (solo en sistemas Unix-like)
                try {
                    process.kill(pid, 0); // No envía señal, solo verifica si existe
                    console.warn(`[WARNING] Proceso con PID ${pid} ya está en ejecución`);
                    return true;
                } catch (e) {
    ...
    async stop() {
        try {
            ...
            console.log(` Deteniendo proceso con PID: ${pid}`);
            
            // Enviar señal SIGTERM para graceful shutdown
            ...
            try {
                process.kill(pid, 0);
                // Si todavía existe, enviar SIGKILL
                console.log('[ALERT] Forzando detención del proceso...');
                process.kill(pid, 'SIGKILL');
            } catch (e) {
                // El proceso ya no existe
                console.log('[OK] Proceso detenido exitosamente');
            }
            ...
            console.error('[ERROR] Error deteniendo proceso:', error);
            return false;
        }
    }
    ...
    showLogs(lines = 20) {
        try {
            if (!fs.existsSync(this.config.logFile)) {
                console.log('[WARNING] No hay archivo de logs');
                return;
            }
            ...
            console.log(` Últimas ${lines} líneas del log:`);
            console.log('=====================================');
            ...
            console.error('[ERROR] Error leyendo logs:', error);
        }
    }
}
...
async function main() {
    ...
    switch (command) {
        ...
        case 'status':
            ...
            console.log('[DATA] Estado del proceso:');
            console.log('======================');
            console.log(`Ejecutando: ${status.running ? '[OK] Sí' : '[ERROR] No'}`);
            if (status.pid) {
        ...
        default:
            ...
            console.log('  logs    - Muestra las últimas líneas del log (opcional: número de líneas)');
            process.exit(1);
    }
}
