/**
 *  MCP CRYPTO NEURAL AUTOMATION
 * ===============================
 * 
 * Sistema automatizado de búsquedas MCP Brave integrado con análisis psicológico neural
 * para potenciar el núcleo de tasas de cambio existente
 */
...
class MCPCryptoNeuralAutomation {
    constructor() {
        console.log(' [MCP NEURAL] Inicializando automation crypto neural...');
    ...
    loadConfig() {
        try {
            ...
            console.error('[ERROR] [CONFIG] Error cargando configuración:', error.message);
    ...
    getDefaultConfig() {
        return {
            ...
            crypto_queries: {
                core_assets: {
                    bitcoin: ['Bitcoin BTC precio análisis técnico'],
                    ethereum: ['Ethereum ETH precio predicción'],
    ...
    async initializeMCPServer() {
        return new Promise((resolve, reject) => {
            console.log(' [MCP SERVER] Iniciando servidor MCP Brave...');
            
            // Verificar si el servidor ya está corriendo
            exec('netstat -an | findstr :9847', (error, stdout) => {
                if (stdout && stdout.includes('9847')) {
                    console.log('[OK] [MCP SERVER] Servidor ya está corriendo en puerto 9847');
                    resolve(true);
                    return;
                }
    ...
    async setupAutomation() {
        console.log(' [AUTOMATION] Configurando automation neural...');
        
        // [ENDPOINTS] QUERIES POR CATEGORÍA
        ...
        //  INICIALIZAR ESTADOS PSICOLÓGICOS
    ...
    setupAdaptiveIntervals() {
        ...
        this.searchIntervals.set('core_assets', baseInterval * 0.8);      // Más frecuente
    ...
    async initializePsychologicalStates() {
        console.log(' [PSYCHOLOGICAL] Inicializando estados psicológicos...');
        ...
        for (const symbol of mainAssets) {
            try {
                ...
                console.log(` [${symbol}] Estado inicial: ${estadoInicial.estado_psicologico.emocion}`);
        ...
        console.log('[OK] [PSYCHOLOGICAL] Estados psicológicos inicializados');
    }
    
    /**
     * [DATA] CREAR DATOS MOCK PARA SÍMBOLO
     */
    ...
    startAutomaticSearches() {
        console.log('[START] [SEARCHES] Iniciando búsquedas automáticas...');
    ...
    /**
     * [TIME] PROGRAMAR BÚSQUEDAS POR CATEGORÍA
     */
    scheduleSearchCategory(category, baseInterval) {
        const executeSearches = async () => {
            try {
                ...
                console.log(`[SEARCH] [${category.toUpperCase()}] Ejecutando búsquedas...`);
                ...
                console.error(`[ERROR] [${category.toUpperCase()}] Error en búsquedas:`, error.message);
            }
        };
    ...
    /**
     *  EJECUTAR BÚSQUEDAS DE ASSET ESPECÍFICO
     */
    async executeAssetSearches(asset, queries, category) {
        console.log(` [${asset.toUpperCase()}] Ejecutando ${queries.length} búsquedas...`);
    ...
    /**
     * [SEARCH] EJECUTAR BÚSQUEDAS GENERALES
     */
    async executeGeneralSearches(queries, category) {
        console.log(`[SEARCH] [${category.toUpperCase()}] Ejecutando ${queries.length} búsquedas generales...`);
    ...
    /**
     * [ENDPOINTS] EJECUTAR BÚSQUEDA INDIVIDUAL
     */
    async executeSearch(query, metadata = {}) {
        try {
            ...
            // Procesar resultado con análisis psicológico
            ...
            console.error(`[ERROR] [SEARCH] Error ejecutando búsqueda "${query}":`, error.message);
    ...
    async processSearchResult(searchResult, query, metadata) {
        try {
            ...
            // Mapear sentiment a estado psicológico
            const psychologicalState = this.mapSentimentToPsychological(sentiment);
            
            // Si es una búsqueda de asset específico, actualizar estado
            if (metadata.asset) {
    ...
    /**
     * [RELOAD] MAPEAR SENTIMENT A ESTADO PSICOLÓGICO
     */
    ...
    /**
     * [DIAMOND] ACTUALIZAR ESTADO PSICOLÓGICO DEL ASSET
     */
    async updateAssetPsychologicalState(asset, searchResult, sentiment) {
        try {
            ...
            if (currentState && currentState.estado_psicologico.emocion !== newState.estado_psicologico.emocion) {
                console.log(`[RELOAD] [${symbol}] Transición psicológica: ${currentState.estado_psicologico.emocion}  ${newState.estado_psicologico.emocion}`);
    ...
    generateRecommendation(psychologicalState, sentiment) {
        const recommendations = {
            ...
            'NEUTRAL': 'Esperar confirmación - Mercado indeciso',
            'PESIMISMO': 'Evaluar oportunidades de compra - Mercado pesimista',
            'PANICO': 'Oportunidad de acumulación - Mercado en pánico'
        };
    ...
    async validateConfiguration() {
        console.log('[OK] [VALIDATION] Validando configuración...');
        
        if (!process.env.BRAVE_SEARCH_API_KEY) {
            throw new Error('BRAVE_SEARCH_API_KEY no configurada');
        }
        
        console.log('[OK] [VALIDATION] Configuración válida');
    }
    ...
    startPsychologicalAnalysis() {
        console.log(' [ANALYSIS] Iniciando análisis psicológico continuo...');
        // Placeholder para análisis continuo
    }
    
    startPerformanceMetrics() {
        console.log('[UP] [METRICS] Iniciando métricas de performance...');
     ...
     */
    stop() {
        console.log(' [SYSTEM] Deteniendo sistema...');
        this.isRunning = false;
