...
class QBTCAdvancedCacheSystem {
    constructor(options = {}) {
        this.options = {
            ...
            staticDataTTL: 3600000,    // 1 hora para datos estáticos
            ...
            minRequestInterval: 2000,   // 2 segundos mínimo entre requests
        ...
        console.log('[START] Advanced Cache System inicializado');
        console.log(`[DATA] Balance TTL: ${this.options.balanceTTL/1000}s`);
        console.log(`[DATA] Positions TTL: ${this.options.positionsTTL/1000}s`);
    }
    ...
    async initializeCache() {
        try {
            ...
            console.log('[OK] Cache system inicializado exitosamente');
            
        } catch (error) {
            console.error('[ERROR] Error inicializando cache system:', error.message);
        }
    }
    ...
    async get(key, fetchFunction, ttl = null) {
        try {
            ...
            this.stats.misses++;
            console.log(`[RELOAD] Cache MISS: ${key} - Fetching fresh data`);
            ...
            if (freshData) {
                ...
                console.log(`[OK] Fresh data cached: ${key}`);
            }
            
            return freshData;
            
        } catch (error) {
            console.error(`[ERROR] Error getting cached data for ${key}:`, error.message);
    ...
    async processRequestQueue() {
        // Verificar si podemos procesar más requests
        ...
        try {
            console.log(`[API] Processing API request: ${request.key} (Queue: ${this.requestQueue.length})`);
            
            const result = await request.fetchFunction();
            request.resolve(result);
            
        } catch (error) {
            console.error(`[ERROR] API request failed for ${request.key}:`, error.message);
            
            // Verificar si es rate limiting
            if (error.message.includes('429') || error.message.includes('rate limit')) {
                this.stats.rateLimitHits++;
                console.log(` Rate limit detected for ${request.key}, extending cache TTL`);
                
                // Intentar devolver datos caducados
                const staleData = this.getStaleData(request.key);
                if (staleData) {
                    console.log(` Using stale data for ${request.key}`);
            ...
            // Procesar siguiente request después de un delay
            setTimeout(() => {
                this.processRequestQueue();
    ...
    async setPersistentCache(key, data, ttl) {
        try {
            ...
            console.error(`[ERROR] Error saving persistent cache for ${key}:`, error.message);
        }
    }
    ...
    getStaleData(key) {
        ...
        if (memoryCached) {
            console.log(` Using stale memory data for ${key}`);
        ...
        try {
            ...
            console.log(` Using stale persistent data for ${key}`);
            return data.data;
        } catch (error) {
            return null;
        }
    ...
    async ensureCacheDirectory() {
        try {
            ...
            console.error('[ERROR] Error creating cache directory:', error.message);
        }
    }
    ...
    async loadPersistentCache() {
        try {
            ...
            console.error('[ERROR] Error loading persistent cache:', error.message);
        }
    }
        ...
        try {
            ...
            for (const file of files) {
                if (file.endsWith('.json')) {
                    ...
                    // Eliminar archivos más viejos que 2 horas
                    if (age > 7200000) {
            ...
            if (cleaned > 0) {
                console.log(` Cleaned ${cleaned} expired files from persistent cache`);
            }
            
        } catch (error) {
            console.error('[ERROR] Error cleaning persistent cache:', error.message);
        }
    }
    ...
    generateReport() {
        ...
        console.log('\n[DATA]  CACHE SYSTEM REPORT ');
        console.log(` Hit Rate: ${stats.hitRate}`);
        console.log(`[UP] Hits: ${stats.hits} | Misses: ${stats.misses}`);
        console.log(`[API] API Calls: ${stats.apiCalls}`);
        console.log(` Rate Limits: ${stats.rateLimitHits}`);
        console.log(` Memory Cache Size: ${stats.memorySize}`);
        console.log(` Queue Size: ${stats.queueSize}`);
        console.log(`[RELOAD] Active Requests: ${stats.activeRequests}`);
        console.log('\n');
