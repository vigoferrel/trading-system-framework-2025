# Sistema de Integracion QBTC - Bots + Trading
# Script para integrar la red de bots con el sistema de trading quantum

Write-Host "=== INTEGRACION QBTC: BOTS + TRADING SYSTEM ===" -ForegroundColor Green
...
# Funcion para verificar estado del sistema
function Get-SystemStatus {
    ...
    try {
        ...
        if ($networkInfo) {
            Write-Host "OK Red Docker: bot-network (192.168.100.0/24)" -ForegroundColor Green
        } else {
            Write-Host "ERROR Red Docker: No encontrada" -ForegroundColor Red
        }
    } catch {
    ...
    foreach ($bot in $botConfigs) {
        try {
            ...
            if ($response.status -eq "running") {
                Write-Host "OK $($bot.Name): $($response.ip) - Activo" -ForegroundColor Green
                $activeBots++
            } else {
                Write-Host "WARN $($bot.Name): $($response.ip) - Estado desconocido" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "ERROR $($bot.Name): Error de conexion" -ForegroundColor Red
        }
    }
...
# Funcion para ejecutar pruebas de integracion
function Test-Integration {
    Write-Host "`n=== PRUEBAS DE INTEGRACION ===" -ForegroundColor Green
    ...
    # Prueba 2: Verificar configuracion de red
    Write-Host "Prueba 2: Configuracion de red..." -ForegroundColor Cyan
    try {
...
# Funcion para mostrar informacion de integracion
function Show-IntegrationInfo {
    Write-Host "`n=== INFORMACION DE INTEGRACION ===" -ForegroundColor Green
    ...
    Write-Host "`n=== COMANDOS DE GESTION ===" -ForegroundColor Yellow
    ...
    Write-Host "Gestion completa: .\qbtc-bot-manager.ps1" -ForegroundColor Cyan

    Write-Host "`n=== INTEGRACION CON TRADING ===" -ForegroundColor Yellow
    ...
    Write-Host "2. Usar .\qbtc-vpn-solution.js para gestion VPN" -ForegroundColor White
    Write-Host "3. Ejecutar .\qbtc-quantum-ip-integration.js" -ForegroundColor White
    Write-Host "4. Los bots estan listos para operaciones distribuidas" -ForegroundColor White
}

# Funcion para ejecutar sistema de trading
function Start-TradingSystem {
    ...
    if (Test-Path "qbtc-quantum-ip-integration.js") {
        Write-Host "Ejecutando integracion quantum..." -ForegroundColor Cyan
...
# Menu principal
do {
    Write-Host "`n=== MENU DE INTEGRACION ===" -ForegroundColor Yellow
    Write-Host "1. Ver estado del sistema" -ForegroundColor White
    Write-Host "2. Ejecutar pruebas de integracion" -ForegroundColor White
    Write-Host "3. Mostrar informacion de integracion" -ForegroundColor White
    ...
    Write-Host "`nSeleccione una opcion (1-6): " -NoNewline
    ...
    switch ($choice) {
        ...
        default { Write-Host "Opcion invalida. Intente nuevamente." -ForegroundColor Red }
    }
...
Write-Host "`n=== INTEGRACION FINALIZADA ===" -ForegroundColor Green
