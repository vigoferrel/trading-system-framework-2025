...
async function testQuantumRecommendations() {
    try {
        console.log(' Testing Enhanced Quantum Recommendations...');
        
        const response = await axios.get('http://localhost:4602/api/quantum-recommendations');
        const data = response.data;
        
        console.log('[OK] Enhanced Quantum Recommendations Response Received');
        console.log(`[DATA] Total Recommendations: ${data.length}`);
        
        if (data.length > 0) {
            console.log('\n[SEARCH] Sample Enhanced Recommendations:');
            
            const sampleRecommendations = data.slice(0, 3);
            
            sampleRecommendations.forEach((rec, index) => {
                console.log(`\n[UP] Recommendation ${index + 1}:`);
            ...
            console.log(`\n[UP] NaN Values Found: ${nanCount}/${data.length} recommendations`);
            
            if (nanCount === 0) {
                console.log(' SUCCESS: All enhanced recommendations are valid!');
            } else {
                console.log('[WARNING]  WARNING: Some NaN values present in recommendations');
            }
            ...
            console.log(`[DATA] Missing Fields: ${missingFields} total missing fields`);
            
            if (missingFields === 0) {
                console.log('[OK] SUCCESS: All required fields present in recommendations');
            } else {
                console.log('[WARNING]  WARNING: Some required fields missing');
            }
            
        } else {
            console.log('[WARNING]  No recommendations found - this might indicate filtering issues');
        }
        
    } catch (error) {
        console.error('[ERROR] Error testing quantum recommendations:', error.message);
        if (error.response) {
