 ...
 * Solución avanzada para prevenir HTTP 429 errors y optimizar llamadas a API
...
class QBTCRateLimitOptimizer {
    ...
    async optimizedRequest(apiType, endpoint, params = {}, options = {}) {
        ...
        if (cachedResult) {
            ...
            console.log(` [${apiType}] Cache hit for ${endpoint}`);
            return cachedResult;
        }
        
        // Verificar rate limit
        if (this.isRateLimited(apiType)) {
            console.log(` [${apiType}] Rate limit reached, queuing request`);
    ...
    isRateLimited(apiType) {
        ...
        // Usar 80% del límite como threshold de seguridad
    ...
    async waitForRateLimit(apiType) {
        ...
        while (this.isRateLimited(apiType)) {
            ...
            if (waitTime > 0) {
                console.log(` [${apiType}] Waiting ${waitTime}ms for rate limit reset`);
        ...
        while (attempt < (options.maxRetries || 3)) {
            try {
                ...
                if (error.message.includes('429') || error.message.includes('Too many requests')) {
                    console.log(` [${apiType}] Rate limit hit, activating backoff`);
                    this.stats.rateLimitHits++;
                    this.stats.backoffActivations++;
                    
                    // Backoff exponencial con jitter
                    const backoffDelay = this.calculateBackoffDelay(delay, attempt);
                    console.log(` [${apiType}] Backing off for ${backoffDelay}ms`);
    ...
    async makeAPICall(apiType, endpoint, params) {
        // Esta función debe ser reemplazada con la implementación real de la API
    ...
    calculateDelay(apiType) {
        const baseDelays = {
            EAPI: 150,  // 150ms para EAPI (más conservador)
    ...
    updateRateLimit(apiType) {
        ...
        if (limit) {
            ...
            // Log cuando nos acercamos al límite
