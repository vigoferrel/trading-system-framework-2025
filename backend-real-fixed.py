#!/usr/bin/env python3
"""
BACKEND REAL QBTC - STACK UNIFICADO (FIXED)
Backend que conecta con servicios reales del sistema QBTC sin problemas de codificación.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('backend_real_fixed.log', encoding='utf-8'),
...
class BackendRealFixed:
    def __init__(self, db_path="backend_real_fixed.db"):
        ...
        self.init_db()
        logger.info("BACKEND REAL QBTC - INICIANDO")
    ...
    async def run_system(self):
        ...
        logger.info("Iniciando sistema con servicios reales...")
    ...
    async def capture_all_data(self):
        ...
        logger.info("Capturando datos reales...")
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.warning(f"Error capturando {ENDPOINTS[i]['service']}/{ENDPOINTS[i]['name']}: {result}")
            else:
    ...
    def show_stats(self, cycle_time: float):
        ...
        if self.start_time:
            ...
            logger.info("=" * 50)
            logger.info(f"ESTADISTICAS BACKEND REAL")
            logger.info(f"Uptime: {uptime_str}")
            logger.info(f"Datos capturados: {self.metrics['captured']}")
            logger.info(f"Errores: {self.metrics['errors']}")
            logger.info(f"Servicios saludables: {self.metrics['healthy_services']}/{len(SERVICES)}")
            logger.info(f"Ciclo: {cycle_time:.2f}s")
            logger.info(f"Datos en cache: {len(self.data_cache)} endpoints")
            logger.info("=" * 50)
    ...
    async def cleanup(self):
        ...
        self.is_running = False
        logger.info("Backend real detenido")

async def main():
    """Función principal."""
    print("=" * 60)
    print("BACKEND REAL QBTC - STACK UNIFICADO (FIXED)")
    print("=" * 60)
    print("Conectando a servicios reales del sistema")
    print("QBTC Core - SRONA API - Frontend - Vigo Futures")
    print("Sin simulaciones - Solo datos reales")
    print("=" * 60)
    
    backend = BackendRealFixed()
