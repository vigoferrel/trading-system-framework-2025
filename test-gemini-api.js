...
async function testGeminiAPI() {
    ...
    const requestBody = {
        ...
        messages: [
            {
                role: 'system',
                content: 'Eres un experto analista de mercados financieros. Responde ÚNICAMENTE con JSON válido, sin markdown.'
            },
            {
                role: 'user',
                content: 'Analiza BTCUSDT y genera una decisión de trading en formato JSON con campos: decision, confidence, reasoning, risk_level, timeframe, key_factors, contradictions_resolved'
            }
        ],
        temperature: 0.3,
        max_tokens: 1000,
    ...
    try {
        ...
        if (response.data && response.data.choices && response.data.choices[0]) {
            ...
            console.log('[ERROR] Respuesta inválida:', response.data);
        }
