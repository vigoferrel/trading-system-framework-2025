...
class FuturesPositionManager {
    ...
    async checkAndCloseAllFuturesPositions() {
        console.log('[SEARCH] Verificando posiciones de futuros abiertas...');
        
        try {
            // Método 1: Obtener balance de futuros para ver posiciones
            const balance = await this.binanceConnector.getAccountBalance();
            console.log('[DATA] Balance actual:', JSON.stringify(balance.__detail, null, 2));
            
            // Método 2: Usar endpoint directo de posiciones de futuros
            const positions = await this.getFuturesPositionsDirectly();
            console.log(`[DATA] Posiciones encontradas: ${positions.length}`);
            
            if (positions.length === 0) {
                console.log('[OK] No hay posiciones de futuros abiertas');
                return;
            }
            
            // Mostrar todas las posiciones
            for (const position of positions) {
                console.log(`[LIST] Posición: ${position.symbol}`);
    ...
    async getFuturesPositionsDirectly() {
        try {
            ...
            console.error('[ERROR] Error obteniendo posiciones directamente:', error.message);
            return [];
        }
    }
    ...
    async closeFuturesPosition(position) {
        try {
            ...
            console.log(`[RELOAD] Cerrando posición: ${symbol} ${side} ${quantity}`);
            ...
            console.log('[LIST] Parámetros de orden:', orderParams);
            
            const result = await this.binanceConnector.placeFuturesOrder(orderParams);
            
            if (result && result.orderId) {
                console.log(`[OK] Posición cerrada exitosamente: ${symbol} - Order ID: ${result.orderId}`);
            } else {
                console.log(`[WARNING] Respuesta inesperada:`, result);
            }
