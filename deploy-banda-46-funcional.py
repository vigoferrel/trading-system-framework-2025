...
def main():
    ...
    for port in ports:
        if check_port(port):
            print(f"[WARNING]  Puerto {port} ya está en uso")
            return
    
    print("[OK] Todos los puertos están disponibles")
    ...
    for service in services:
        ...
        try:
            ...
            if wait_for_service(service['url']):
                print(f"[OK] {service['name']} está respondiendo")
            else:
                print(f"[WARNING]  {service['name']} no responde, pero continúa...")
    ...
    print(" Visualizador neuronal: http://localhost:4606")
    print(" Cerebro maestro LLM: http://localhost:4607")
    print("=" * 60)
