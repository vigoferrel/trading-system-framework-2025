...
// Importaciones simuladas para el propósito de este script modular.
...
class QuantumCopilotPlayer {
    ...
    revealStrategy(collapsed) {
        return {
            ...
            reasoning: [
                `[ENDPOINTS] Posicion sugerida: ${collapsed.position} en ${collapsed.strategy.instrument}`,
                `[FAST] Confianza cuantica: ${(collapsed.confidence * 100).toFixed(1)}%`,
                ` Efecto entrelazamiento: ${collapsed.entanglementEffect}`,
                `[DIAMOND] Quanta ganados por la incertidumbre: +${collapsed.wisdomQuanta.toFixed(4)}`,
                `[UP] Profit Potencial: ${collapsed.strategy.riskManagement.takeProfit}`,
                `[DOWN] Riesgo Maximo: ${collapsed.strategy.riskManagement.stopLoss}`
            ],
